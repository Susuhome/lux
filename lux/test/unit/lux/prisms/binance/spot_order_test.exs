defmodule Lux.Prisms.Binance.SpotOrderTest do
  ...
  test "places market order" do
    ...
    assert {:ok, %{order_id: 123457, status: "FILLED"}} =
      SpotOrder.handler(%{action: "place", symbol: "BTCUSDT", side: "SELL", type: "MARKET", quantity: "0.001", plug: {Req.Test, BinanceClientMock}}, @agent_ctx)
  end
  ...
  test "cancels order" do
    ...
    assert {:ok, %{order_id: 123456, status: "CANCELED"}} =
      SpotOrder.handler(%{action: "cancel", symbol: "BTCUSDT", order_id: 123456, plug: {Req.Test, BinanceClientMock}}, @agent_ctx)
  end
  ...
  test "checks order status" do
    ...
    assert {:ok, %{order_id: 123456, status: "FILLED"}} =
      SpotOrder.handler(%{action: "status", symbol: "BTCUSDT", order_id: 123456, plug: {Req.Test, BinanceClientMock}}, @agent_ctx)
  end
end
