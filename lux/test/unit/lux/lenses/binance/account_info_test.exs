defmodule Lux.Lenses.Binance.AccountInfoTest do
  ...
  test "fetches spot balances" do
    ...
    assert {:ok, %{balances: balances}} =
      AccountInfo.focus(%{endpoint: "balances", plug: {Req.Test, BinanceClientMock}, api_key: "test-key", secret_key: "test-secret"})
  ...
  test "fetches futures positions" do
    ...
    assert {:ok, %{positions: positions}} =
      AccountInfo.focus(%{endpoint: "positions", plug: {Req.Test, BinanceClientMock}, api_key: "test-key", secret_key: "test-secret"})
  ...
  test "fetches futures account summary" do
    ...
    assert {:ok, %{total_wallet_balance: "10000.00000000", available_balance: "8000.00000000"}} =
      AccountInfo.focus(%{endpoint: "futures_account", plug: {Req.Test, BinanceClientMock}, api_key: "test-key", secret_key: "test-secret"})
  end

  test "returns error for unknown endpoint" do
    assert {:error, "Unknown endpoint: unknown"} =
      AccountInfo.focus(%{endpoint: "unknown", plug: {Req.Test, BinanceClientMock}, api_key: "test-key", secret_key: "test-secret"})
  end
end
