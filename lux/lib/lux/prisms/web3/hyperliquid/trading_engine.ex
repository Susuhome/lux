defmodule Lux.Prisms.Web3.Hyperliquid.TradingEngine do
  @moduledoc """
  Hyperliquid perpetual trading: orders, positions, leverage, risk monitoring.
  """
  ...
  def handle_call({:place_order, params}, _from, state) do
    ...
    if leverage > state.config.max_leverage do
      {:reply, {:error, :leverage_exceeded}, state}
    else
      id = gen_id()
      order = %{
        id: id, symbol: params[:symbol], side: params[:side],
        type: params[:type] || :limit, price: params[:price],
        size: params[:size], leverage: leverage,
  ...
  def handle_call(:pnl_summary, _from, state) do
    ...
    {:reply, {:ok, %{total_pnl: total, trades: length(state.pnl_history), wins: wins, losses: losses}}, state}
  end
